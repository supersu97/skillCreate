...
## 步骤0：交互式输入选择（新增）
当用户未明确指定输入方式时，必须使用交互式选择，执行以下流程：

1. **欢迎信息**（简要说明技能功能）
   ```
   欢迎使用Java后台代码修改助手！
   我可以帮助你修改Spring框架、MyBatis、JUnit测试等Java后台代码。
   ```

2. **使用 AskUserQuestion 工具提供输入方式选择菜单**
   ```javascript
   {
     "questions": [{
       "question": "请选择修改方式（单选）：",
       ...
       "options": [
         {
           "label": "直接描述进行修改",
           "description": "直接描述要修改的内容和要求，我会根据你的描述进行修改"
         },
         {
           "label": "根据接口文档进行修改",
           "description": "提供本地或在线接口文档，我会根据文档更新代码"
         },
         {
           "label": "根据指定方法或接口进行修改",
...
3. **根据用户选择收集相应信息**
   ...
   - 如果选择"根据接口文档进行修改"：
     1. 首先询问："请提供接口文档链接或本地文件路径："
     2. 然后询问："请描述在哪一块根据接口文档修改什么，或者根据接口文档新增什么接口："
   - 如果选择"根据指定方法或接口进行修改"：
     1. 首先询问："请指定要修改的具体方法、接口或文件："
     2. 然后询问："请描述要修改什么内容或具体要求："
   - 如果选择"直接新增代码"：
     1. 首先询问："（可选）如果有相关的接口文档，请提供链接或本地路径（如无则留空）："
     2. 然后询问："请描述要新增的代码功能、位置和具体实现要求："
   - 如果选择"修复bug或安全问题"，询问："请描述要修复的问题或漏洞："
   - 如果选择"优化性能或重构"，询问："请描述要优化或重构的代码："

4. **确认修改需求**（关键步骤）
   收集完信息后，根据用户选择的修改方式总结需求并请求确认：

   **根据用户选择的修改方式，总结相应的信息：**
   - 如果选择"直接描述进行修改"：总结修改内容和要求
   - 如果选择"根据接口文档进行修改"：总结接口文档路径和在哪一块根据文档修改/新增的内容
   - 如果选择"根据指定方法或接口进行修改"：总结指定的具体方法/接口和要修改的内容/要求
   - 如果选择"直接新增代码"：总结（可选的）接口文档和新增代码的具体要求
   - 如果选择"修复bug或安全问题"：总结要修复的问题描述
   - 如果选择"优化性能或重构"：总结要优化或重构的代码描述
...
## 步骤1：理解修改需求
- 根据交互式输入选择的结果，识别要修改的目标（类名、方法名、文件路径等）
- 理解修改的具体要求和目的
- 确定是否需要生成JUnit测试
...
当你直接调用本技能而不指定具体修改方式时，系统会自动进入交互式输入流程：

1. **选择修改方式**：系统会显示修改方式选择菜单，包括：
   - 直接描述进行修改
   - 根据接口文档进行修改
   - 根据指定方法或接口进行修改
   - 直接新增代码
...
2. **分步提供详细信息**：根据你的选择，系统会分步询问相应的详细信息
   - **直接描述的**：直接描述要修改的内容和要求
   - **选择方法或接口的**：先填具体方法跟接口，再填要修改什么
   - **选择接口文档的**：先填接口文档地址，再填在哪一块根据接口文档修改什么，或者根据接口文档新增什么接口
   - **直接新增的**：（可选）提供接口文档地址，再填新增什么功能
   - **修复bug、优化性能的**：直接描述问题或需求

3. **确认修改需求**：系统会总结你的需求并请求确认
4. **执行修改**：按照你确认的需求进行代码修改
...
1. **交互式输入收集**：首先通过交互式菜单收集修改需求
   - 选择修改方式（直接描述、根据文档、指定方法、直接新增等）
   - 分步提供详细信息（根据选择方式提供相应的一步或两步信息）
   - 确认修改需求
