...
本模块负责后端项目信息的缓存管理，包括缓存检查、读取和保存。

## 缓存位置

**统一使用默认位置**：`~/.claude/cache/backend-projects/[项目路径 hash]/`
...
// 构建默认缓存基础路径
const cacheBasePath = $HOME/.claude/cache/backend-projects/[项目路径 hash]
```
...
2. **检查缓存是否存在且有效**
   ...
   // 2. 构建缓存路径
   const cacheBasePath = $HOME + '/.claude/cache/backend-projects/' + projectHash
...
```json
{
  ...
  "projectInfo": {
    ...
    "projectType": "backend",
    "framework": "[框架名称和版本]",
    "language": "[Java/Python/Go 等]",
    "database": "[MySQL/PostgreSQL/MongoDB 等]",
    "orm": "[JPA/MyBatis/Hibernate 等]",
    "packageManager": "maven/gradle/pip",
    "entryFile": "pom.xml/build.gradle/requirements.txt",
    "srcDirectory": "src",
    "commonPaths": {
  ...
  "codeStyle": {
    ...
    "controllerPattern": "[Controller 命名和基类模式]",
    "responsePattern": "[响应封装模式]",
    "paramPattern": "[参数获取模式]",
    "loggingPattern": "[日志记录模式]",
    "exceptionHandling": "[异常处理模式]"
  },
  "sampleFiles": [
...
3. **创建缓存目录**
   ```javascript
   调用 Bash 工具：
   mkdir -p "$HOME/.claude/cache/backend-projects/[projectHash]"
   ```
