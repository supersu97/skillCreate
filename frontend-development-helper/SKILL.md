---
name: "frontend-development-helper"
description: "前端开发助手。支持 Vue、React、Angular、Taro 等主流前端框架的开发任务。当需要修改前端代码、新增前端页面/组件、修复前端 Bug 或优化前端性能时调用。"
---

# 前端开发助手

*本技能专为 Claude Code 设计，帮助开发者处理前端项目的开发任务。*
...
| [project-collector.md](project-collector.md) | 前端项目位置收集模块 |
| [cache-handler.md](cache-handler.md) | 缓存处理模块 |
| [project-analyzer.md](project-analyzer.md) | 前端项目分析模块 |
| [requirement-collector.md](requirement-collector.md) | 前端需求收集模块 |
...
**必须先使用 Read 工具读取 `[技能基础路径]/project-analyzer.md` 文件**，按照"前端项目结构分析"部分执行。

#### 3.3 分析代码风格

**如果使用了缓存，跳过本步骤**

**必须先使用 Read 工具读取 `[技能基础路径]/project-analyzer.md` 文件**，按照"前端代码风格分析"部分执行。

生成前端代码风格指南，后续所有代码生成必须严格遵循。
...
如果需求涉及 UI 设计图，自动调用 ui-design-handler.md。
...
{
  "taskId": [主任务ID],
  "description": "步骤6完成：前端代码修改 - [修改文件列表]"
}
```
