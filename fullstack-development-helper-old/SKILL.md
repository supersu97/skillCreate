...
调用 Glob 工具：
- path: [前端项目路径]
- pattern: "**/*.test.*" 或 "**/*.spec.*" 或 "**/__tests__/**/*" 或 "**/test/**/*"
...
- 测试文件（如：src/components 下的 .test.* 或 .spec.* 文件）
...
调用 Glob 工具：
- path: [后端项目路径]
- pattern: "**/*Test.*" 或 "**/*Spec.*" 或 "**/test/**/*" 或 "**/tests/**/*"
...
- 测试文件（如：*Test.java、*Spec.js、test_*.py）
```
...
#### 4. 测试代码风格（可选）
- 测试框架：[如：Jest、Vitest、Cypress、Testing Library]
- 测试文件命名：[如：UserList.test.js、UserDetail.spec.ts]
- 测试结构：[如：describe/it、test/suite、beforeEach/afterEach]
- 断言库：[如：expect、assert、should]
...
#### 4. 测试代码风格（可选）
- 测试框架：[如：JUnit、pytest、Jest、Mocha]
- 测试类命名：[如：UserServiceTest、UserControllerSpec]
- 测试方法命名：[如：testGetUserById、shouldReturnUserList]
- 测试注解：[如：@Test、@BeforeEach、@AfterEach]
- 断言方式：[如：assertThat、assertEquals、assertTrue]
...
- **修改范围限制**：严格遵循用户指定的修改范围，没有要求改动的地方必须保持原样，不得进行额外修改
...
```
## 前后端联动修改流程（Agent Team协作模式）

### 第一步：收集完整修改需求
在执行任何修改之前，先收集完整的全栈修改需求：
1. **功能需求**：详细描述要实现的功能
2. **接口规范**：如果需要新接口或修改现有接口，提供接口设计
3. **前端修改点**：明确前端需要修改的具体内容
4. **后端修改点**：明确后端需要修改的具体内容
5. **数据变更**：如有数据库变更，提供SQL语句
...
### 第二步：创建Agent Team并分配任务
基于完整的修改方案文档，创建专业Agent团队：

**Agent团队组成**：
1. **接口协调Agent** - 负责接口规范和前后端协调（如果需要）
2. **后端开发Agent** - 负责后端代码修改
3. **前端开发Agent** - 负责前端代码修改

**任务分配**：
- 将完整的修改方案文档分发给所有Agent
- 每个Agent根据文档中自己的部分执行修改任务
- Agent之间基于文档进行协调，无需重复沟通

### 第三步：Agent并行执行修改
**后端开发Agent执行流程**：
1. 基于修改方案文档中的后端修改清单
2. 执行后端代码修改（参考"执行后端代码修改"流程）
3. 确保实现与接口规范一致

**前端开发Agent执行流程**：
1. 基于修改方案文档中的前端修改清单
2. 执行前端代码修改（参考"执行前端代码修改"流程）
3. 确保实现与接口规范一致

### 第四步：协调与验证
- Agent之间基于统一的修改方案文档进行协调
- 验证前后端实现的一致性
- 如有问题，参考修改方案文档进行调整

### Agent Team协作优势
1. **需求先行**：先收集完整需求，避免重复沟通
2. **专业分工**：每个Agent专注于自己的专业领域
3. **高效并行**：基于统一文档，前后端同时执行修改
4. **一致性保证**：所有修改基于同一份方案文档，确保前后端一致
```
...
```javascript
{
  "questions": [{
    ...
    "options": [
      ...
      {
        "label": "Agent Team协作模式",
        "description": "先收集完整需求，创建Agent团队并行执行修改"
      }
    ]
  }]
}
...
根据用户在步骤4.2中选择的协同模式，执行相应的流程：

**模式A：后端先改模式**

当用户选择"后端先改模式"时，按照以下流程执行：
...
先收集后端的完整修改需求：
1. 功能需求：详细描述要实现的后端功能
2. 接口设计：如果需要新接口或修改现有接口，提供接口设计
3. 数据变更：如有数据库变更，提供SQL语句
4. 具体修改：明确后端需要修改的文件、类、方法
...
当用户选择"Agent Team协作模式"时，按照以下流程执行：
...
- 如果选择"根据后端接口路径进行修改"：
  1. 首先询问："请提供需要修改的后端接口信息："
     ...
     - 接口方法（如：GET、POST、PUT、DELETE）
     - 或后端方法名（如：UserController.getUserById）
  2. 然后询问："请描述要根据这个后端接口修改前端的什么内容："
     - 需要修改哪个前端API服务调用
     - 需要更新哪些页面组件的数据处理
     - 需要调整哪些请求参数或响应处理
- 如果选择"根据接口文档进行修改"：
  1. **引用步骤2已收集的接口文档信息**，首先提示用户：
     ```
     根据步骤2的需求收集，您已提供的接口文档信息：
     - 接口文档类型：[本地文件/在线链接/暂无]
     - 接口文档路径/链接：[用户提供的路径或链接]
     
     基于此接口文档，请描述要修改的前端API调用或数据处理：
     ```
  2. 然后询问："请描述在哪一块根据接口文档修改什么，或者根据接口文档新增什么API调用："
- 如果选择"根据指定组件或文件进行修改"：
  1. 首先询问："请指定要修改的具体组件、页面或文件："
  2. 然后询问："请描述要修改什么内容或具体要求："
- 如果选择"直接新增组件或页面"：
  1. **（可选）引用步骤2已收集的接口文档信息**，如果步骤2有提供接口文档，则提示：
     ```
     根据步骤2的需求收集，您已提供的接口文档信息：
     - 接口文档类型：[本地文件/在线链接/暂无]
     - 接口文档路径/链接：[用户提供的路径或链接]
     ...
     将基于此接口文档进行前端组件新增。
     请描述要新增的组件功能、位置和具体实现要求：
     ```
  2. 如果没有接口文档，直接询问："请描述要新增的组件功能、位置和具体实现要求："
- 如果选择"修复bug或样式问题"，询问："请描述要修复的问题："
- 如果选择"优化性能或用户体验"，询问："请描述要优化的内容："
...
1. 修改方式：直接描述进行修改
2. 修改内容：[用户描述的内容]
3. 代码风格：严格遵循步骤1.5中的前端代码风格
4. 接口文档：基于步骤2的接口文档信息（如有）
...
2. 后端接口信息：
   ...
   - 接口方法：[用户提供的HTTP方法]
   - 或后端方法名：[用户提供的后端方法名]
3. 前端修改内容：[用户描述的前端修改要求]
4. 代码风格：严格遵循步骤1.5中的前端代码风格
...
```
根据您的输入，我理解您希望前端：
1. 修改方式：根据指定组件或文件进行修改
2. 指定目标：[用户指定的组件/页面/文件]
3. 修改内容：[用户描述的具体要求]
4. 代码风格：严格遵循步骤1.5中的前端代码风格
...
**情况E：其他修改方式（新增代码、修复bug、优化重构）**
```
根据您的输入，我理解您希望前端：
1. 修改方式：[修改方式]
2. 具体信息：[根据修改方式总结的关键信息]
3. 代码风格：严格遵循步骤1.5中的前端代码风格
4. 接口文档引用（如有）：基于步骤2的接口文档信息
...
**情况1：如果选择"根据后端接口路径进行修改"**
1. 使用 Grep 工具根据接口路径查找前端对应代码：
   ```javascript
   调用 Grep 工具：
   - path: [前端项目路径]
   - pattern: "[用户提供的接口路径]"
...
2. 查找API服务定义文件：
   ...
   - pattern: "**/services/**/*.ts"
   ```
3. 分析找到的代码，确定修改位置和影响范围

**情况2：如果选择"根据指定组件或文件进行修改"**
1. 使用 Glob 工具查找目标文件
2. 使用 Read 工具读取相关代码
3. 分析组件依赖关系和样式依赖
...
**第六步：前端修改执行方式选择**
使用 AskUserQuestion 工具提供选择：
```javascript
{
  "questions": [{
    "question": "请选择前端执行操作（可多选）：",
    "header": "前端操作",
    "multiSelect": true,
    "options": [
      {
      ...
      {
        "label": "生成组件测试",
        "description": "为修改的组件生成单元测试"
      },
      {
        "label": "只修改指定组件",
        "description": "只修改特定的组件，不修改其他部分"
      }
    ]
  }]
}
...
- 使用 Write 工具创建新文件（如组件文件、测试文件）
- 严格遵循步骤1.5中的前端代码风格
- 记录每个修改操作的结果
- **代码修改限制**：严格遵循用户指定的修改范围，没有要求改动的地方必须保持原样，不得进行额外修改

当用户选择"执行后端代码修改"时，必须按照以下流程执行：
...
```javascript
{
  "questions": [{
    ...
    "options": [
      ...
      {
        "label": "根据接口文档进行修改",
        "description": "提供本地或在线接口文档，根据文档更新代码"
      },
      {
        "label": "根据指定方法或接口进行修改",
        "description": "指定具体的方法、接口或文件，然后说明要修改的内容"
      },
      {
        "label": "直接新增代码",
        "description": "添加新的方法、接口、类或功能"
      },
      {
        "label": "修复bug或安全问题",
        "description": "修复代码中的错误、漏洞或安全问题"
      },
      {
        "label": "优化性能或重构",
        "description": "优化代码性能、进行重构或改进代码结构"
      }
    ]
  }]
}
...
- 如果选择"根据接口文档进行修改"：
  1. **引用步骤2已收集的接口文档信息**，首先提示用户：
     ...
     基于此接口文档，请描述要修改的具体内容：
     ```
  2. 然后询问："请描述在哪一块根据接口文档修改什么，或者根据接口文档新增什么接口："
- 如果选择"根据指定方法或接口进行修改"：
  1. 首先询问："请指定要修改的具体方法、接口或文件："
  2. 然后询问："请描述要修改什么内容或具体要求："
- 如果选择"直接新增代码"：
  1. **（可选）引用步骤2已收集的接口文档信息**，如果步骤2有提供接口文档，则提示：
     ```
     ...
     将基于此接口文档进行代码新增。
     请描述要新增的代码功能、位置和具体实现要求：
     ```
  2. 如果没有接口文档，直接询问："请描述要新增的代码功能、位置和具体实现要求："
- 如果选择"修复bug或安全问题"，询问："请描述要修复的问题或漏洞："
- 如果选择"优化性能或重构"，询问："请描述要优化或重构的代码："
...
```
根据您的输入，我理解您希望后端：
1. 修改方式：根据指定方法或接口进行修改
2. 指定目标：[用户指定的方法/接口/文件]
3. 修改内容：[用户描述的具体要求]
4. 代码风格：严格遵循步骤1.5中的后端代码风格
...
- 考虑向后兼容性
- 考虑性能和安全影响
- **数据库修改处理**：如果修改涉及数据库表结构或数据变更，生成相应的SQL语句供用户自行执行，技能不直接操作数据库
...
```javascript
{
  "questions": [{
    ...
    "options": [
      ...
      {
        "label": "生成数据库变更SQL",
        "description": "生成数据库表结构或数据变更的SQL语句，供用户自行执行"
      },
      {
        "label": "只修改指定方法",
...
- 记录每个修改操作的结果
- **代码修改限制**：严格遵循用户指定的修改范围，没有要求改动的地方必须保持原样，不得进行额外修改
- **数据库操作限制**：技能不直接执行数据库操作。如果修改涉及数据库变更，生成相应的SQL语句，由用户自行在数据库中执行。SQL语句格式参考步骤3.2中的数据库变更部分
...
7. **测试**：建议生成相应的测试代码
8. **修改范围**：严格遵循用户指定的修改范围，没有要求改动的地方必须保持原样，不得进行额外修改
